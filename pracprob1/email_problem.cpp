...
void process_file(std::ifstream &infile, std::array<std::string, arrMaxSize> &validEmails, std::array<std::string, arrMaxSize> &invalidEmails, int &goodCount, int &invalidCount);
...
int main()
{
    ...
    std::array<std::string, arrMaxSize> validEmails;
    std::array<std::string, arrMaxSize> invalidEmails;
    int goodCount = 0;
    int invalidCount = 0;

    process_file(emailList, validEmails, invalidEmails, goodCount, invalidCount);
    emailList.close();

    std::cout << "Valid Emails: " << std::endl;
    print(validEmails, goodCount);
...
void process_file(std::ifstream &infile, std::array<std::string, arrMaxSize> &validEmails, std::array<std::string, arrMaxSize> &invalidEmails, int &goodCount, int &invalidCount)
{
    ...
    while (infile)
    {
        if (has_at(email))
        {
            if (is_valid(email))
            {
                validEmails.at(goodCount) = email;
                goodCount++;
            }
            else
            {
